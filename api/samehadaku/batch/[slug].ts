...
const ENDPOINT = "/api/samehadaku/batch/:slug";
...
const parseBatch = async (slug: string) => {
  ...
  const batchItems = extractListItems(batchSection);
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape Samehadaku batch";
