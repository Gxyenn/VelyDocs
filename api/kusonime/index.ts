...
const parseHomepage = async () => {
  ...
  const recommendedSection = extractSection(html, "recomend") || extractSection(html, "recommended") || html;
  const recommended = extractListItems(recommendedSection)
    .map((item) => {
      const itemHtml = item[1];
      const anchorMatch = extractFirstAnchor(itemHtml);
      const anchorTag = anchorMatch?.[0] ?? "";
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape Kusonime homepage";
