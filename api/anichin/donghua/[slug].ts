...
const BASE_URL = "https://anichin.top";
const SOURCE = "Anichin";
const ENDPOINT = "/api/anichin/donghua/:slug";

const resolveAnimeUrl = (slug: string) => {
  if (slug.startsWith("http")) return slug;
  if (slug.startsWith("donghua/")) return `${BASE_URL}/${slug}`;
  if (slug.includes("/")) return `${BASE_URL}/${slug}`;
  return `${BASE_URL}/donghua/${slug}/`;
};
...
const parseDonghuaDetail = async (slug: string) => {
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const data = await parseDonghuaDetail(slug);
    return successResponse(SOURCE, ENDPOINT, data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to scrape Anichin donghua detail";
