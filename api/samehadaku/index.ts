...
const ENDPOINT = "/api/samehadaku";

const parseHomepage = async () => {
  const html = await fetchHtml(BASE_URL);
  const latestSection = extractSection(html, "listupd") || html;
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape Samehadaku homepage";
