...
const ENDPOINT = "/api/otakudesu/search";
...
const parseSearch = async (query: string, page: number) => {
  const url =
    page > 1
      ? `${BASE_URL}/page/${page}/?s=${encodeURIComponent(query)}&post_type=anime`
      : `${BASE_URL}/?s=${encodeURIComponent(query)}&post_type=anime`;
  const html = await fetchHtml(url);
  const section = extractSection(html, "venutama") || html;
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const data = await parseSearch(query, currentPage);
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape Otakudesu search";
