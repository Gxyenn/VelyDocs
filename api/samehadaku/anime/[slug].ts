...
const ENDPOINT = "/api/samehadaku/anime/:slug";
...
const parseInfoMap = (html: string) => {
  const infoSection = extractSection(html, "infoanime") || html;
...
const parseAnimeDetail = async (slug: string) => {
  ...
  const synopsis =
    toText(jsonLd?.description) || toText(extractByClass(html, "sinopsis") || "");

  const score =
    toText(html.match(/class=["'](?:rating|score|imdb)["'][^>]*>([\s\S]*?)<\/[^>]+>/i)?.[1]) ||
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape Samehadaku anime detail";
