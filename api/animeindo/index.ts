...
const BASE_URL = "https://animeindo.my.id";
const SOURCE = "AnimeIndo";
const ENDPOINT = "/api/animeindo";
...
const parseHomepage = async () => {
  ...
  const ongoingSection = extractSection(html, "ongoing") || html;
  const ongoing = extractListItems(ongoingSection)
    .map((item) => {
      const itemHtml = item[1];
      const anchorMatch = extractFirstAnchor(itemHtml);
      const anchorTag = anchorMatch?.[0] ?? "";
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to scrape AnimeIndo homepage";
