...
const ENDPOINT = "/api/otakudesu/completed";
...
const parseCompleted = async (page: number) => {
  ...
  const results = items
    .map((item) => {
      ...
      const score = toText(extractByClass(itemHtml, "score") || extractByClass(itemHtml, "rate"));
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const data = await parseCompleted(currentPage);
    return success(SOURCE, ENDPOINT, data.results, {
      page: currentPage,
      total: data.totalPages,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to scrape Otakudesu completed list";
